...
export default function RegisterPage() {
  ...
  const authCompletePath = nextPath
    ? `/auth/complete?next=${encodeURIComponent(nextPath)}`
    : "/auth/complete";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await register(email, password, name, authCompletePath);
      if (!user) {
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
          ...
          {!notice && (
            <div className="mt-6 text-center text-sm text-gray-500">
              すでにアカウントをお持ちの方は{" "}
              <Link href={loginHref} className="text-teal-600 font-medium hover:underline">
                ログイン
              </Link>
            </div>
          )}
